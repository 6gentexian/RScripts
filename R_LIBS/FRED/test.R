...
###################################################
library(tseries)
GSPC <- as.xts(get.hist.quote("^GSPC",start="2005-01-02",
...
addAvgPrice(on=1)  ##  on=1 where to draw, on chart?
addT.ind()
...
data.model <- specifyModel(T.ind(GSPC) ~ #Delt(Cl(GSPC)[, 1], k=1:10) +
       myATR(GSPC)  + myADX(GSPC) + myAroon(GSPC) +      # + mySMI(GSPC)  myBB(GSPC)  +
       myChaikinVol(GSPC) + myCLV(GSPC) +
       myEMV(GSPC) + #EMA(Delt(Cl(GSPC)[, 1]))  +   CMO(Cl(GSPC)[, 1])  +
       myVolat(GSPC)  + myMACD(GSPC) + myMFI(GSPC) + #RSI(Cl(GSPC)[, 1]) +
